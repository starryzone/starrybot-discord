...
const isValidSignature = async (signed, signature, publicKey, address) => {
  ...
  try {
    ...
    const expectedAddress = new Bech32Address(
      cryptoPubKey.getAddress()
    ).toBech32("cosmos");
...
async function tokenRuleInfo(body, client) {
  ...
  const cosmosHubAddress = await db.getCosmosHubAddress({
    guildId,
    discordUserId,
  });
  console.log(
    `Guild ${guildId} has user ${discordUserId} whose Cosmos Hub address is ${cosmosHubAddress}`
  );
    ...
    if (!roleDiscord) {
      ...
      if (discordRole && !member.roles.cache.has(discordRole.id)) {
        ...
        try {
          ...
          logger.error(
            `Error trying to add role. Guild: ${guild.id}, User: ${author.id}`,
            e
          );
          return;
        }
